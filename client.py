...
def decode_resp(resp):
    ...
    if split[0] == "hi":
        ...
        datastr = split[3]
        ret["data"] = [
            int(datastr[i * 2 : i * 2 + 2], 16) for i in range(0, int(len(datastr) / 2))
        ]
...
async def socketcand_init(reader, writer):
    ...
    try:
        while True:
            ...
            for r in resp:
                event = decode_resp(r)
                state_socketcand = await socketcand_init_statemachine(
                    state_socketcand, writer, event
                )
...
async def socketcand_tx(writer, frame):
    ...
    out_str = out_str + " ".join(
        [idstr, str(len(frame["data"])), " ".join([f"{i:02X}" for i in frame["data"]])]
    )
    out_str = out_str + " >"
...
async def main():
    ...
    await asyncio.gather(
        socketcand_client(reader, writer, bus), kvaser_client(bus, writer)
    )
